...
AI was used throughout, per the case study's encouragement, with human review at each step:

- **Code generation** (schema, cleaning pipeline, generator) was AI-assisted and reviewed
  line by line; the cleaning rules and dedup strategy were directed and corrected by hand.
- **The one-pager's narrative** is generated by OpenAI at runtime, but constrained to
  computed facts (see the architecture note above).

Full transparency notes are in `RATIONALE.docx`.
